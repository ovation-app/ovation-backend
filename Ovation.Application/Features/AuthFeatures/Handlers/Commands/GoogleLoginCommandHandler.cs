...
{
    internal class GoogleLoginCommandHandler(IServiceScopeFactory serviceScopeFactory) : BaseHandler(serviceScopeFactory), IRequestHandler<GoogleLoginCommandRequest, UserToken>
    {
        public async Task<UserToken> Handle(GoogleLoginCommandRequest request, CancellationToken cancellationToken)
        {
            var user = Guid.Empty;
            try
            {
                var verifyDetails = await _userRepository.GoogleLoginAsync(request.Email);

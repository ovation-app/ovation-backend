...
{
    internal class XLoginCommandHandler(IServiceScopeFactory serviceScopeFactory) : BaseHandler(serviceScopeFactory), IRequestHandler<XLoginCommandRequest, UserToken>
    {
        public async Task<UserToken> Handle(XLoginCommandRequest request, CancellationToken cancellationToken)
        {
            var user = Guid.Empty;
            try
            {
                var verifyDetails = await _userRepository.XLoginAsync(request.XId);
